...
namespace RestaurantReviews.WebApp.Controllers
{
    public class ReviewController : Controller
    {
        ...
        public IActionResult Edit(Review viewModel)
        {
            ...
            try
            {
                _repo.EditReview(viewModel.Id, viewModel, true);
                Log.Debug("Review has been edited! " + viewModel.Title);
        ...
        [HttpGet]
        [HttpPost]
        public IActionResult Delete(Review viewModel)
        {
            Review returnreview = viewModel;
            if (!ModelState.IsValid)
            {
                return View(viewModel);
            }
            try
            {
                _repo.EditReview(viewModel.Id, viewModel, false);
                Log.Debug("Review has been deleted! " + viewModel.Title);
            }
            catch (Exception e)
            {
            ...
            return View("Details", returnreview);
        }
        [HttpGet]
        ...
        public IActionResult Create(CreatedReview viewModel)
        {
            ...
            try
            {
                ...
                Log.Debug("Review has been created! " + viewModel.Title);
